...
# this demonstrates the effect deadlines have on the recovery from overload
...
sub tb-producer() {
    ...
    return (random-poisson-distance($interval), { id => $i++, start-at => $*current-time });
}

sub p2-processor($message) {
    if $message{'start-at'} < ($*current-time - 250) {
        say "message with age {$*current-time - $message{'start-at'}} timed out";
        &*fail($message);
        return (random-poisson-distance(0), "");
    }
    else {
        say "message age {$*current-time - $message{'start-at'}}";
        return (random-poisson-distance(240), $message);
    }
}

my $example = TestBed.new(producer-func => &tb-producer);
my $q1 = Queue.new(id => "q1", size => 0);
...
@results = apply-aggregate(@results, {
    ...
    fails => sub (@vals) { # XXX same as above refactor
        my $mean = ([+] @vals).Num / @vals.elems;
        my $dev = Nil;
        if (@vals.elems > 1) {
            $dev = sqrt([+] @vals.map({ ($_ - $mean) ** 2 })).Num / (@vals.elems - 1);
        }
        return fails_mean => $mean, fails_dev => $dev;
    },
    tb => sub (@vals) {
        return {};
    }
...
my $result-fh = open("deadlines.data", :w);
